...
/** Version */
#define HAL_TIMER_VERSION (0x00000400) // 0.4.0
...
#define ENABLE_TIMER(timerNum)  if((timerNum) < HAL_NUM_TIMERS) { VOR_TIM_BANK[timerNum]->ENABLE = 1; }
#define DISABLE_TIMER(timerNum) if((timerNum) < HAL_NUM_TIMERS) { VOR_TIM_BANK[timerNum]->ENABLE = 0; }
...
extern hal_status_t HAL_Timer_SetupPeriodicIrqMs(uint32_t timerNum, uint32_t periodMsec, uint32_t priority);
extern hal_status_t HAL_Timer_SetupPeriodicIrqUs(uint32_t timerNum, uint32_t periodUsec, uint32_t priority);
extern hal_status_t HAL_Timer_SetupTimeoutMs(uint32_t timerNum, uint32_t timeoutMsec);
extern hal_status_t HAL_Timer_SetupTimeoutUs(uint32_t timerNum, uint32_t timeoutUsec);
extern hal_status_t HAL_Timer_SetDelayTimer(uint32_t timerNum);
extern hal_status_t HAL_Timer_DelayMs(uint32_t delayMsec);
extern hal_status_t HAL_Timer_SetupPWMA(uint32_t timerNum, uint32_t freqHz, percent_1000_t dutyCycle);

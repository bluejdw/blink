...
typedef enum {
  en_spi_state_idle = 0,
  en_spi_state_pending,
  en_spi_state_finished
} en_spi_state_t;
...
static volatile en_spi_state_t states[NUM_SPI_BANKS] = {en_spi_state_idle,
                                                        en_spi_state_idle,
                                                        en_spi_state_idle,
                                                        en_spi_state_idle};
...
__STATIC_INLINE void InitCircularBuffer(uint32_t bank)
{
  VOR_CircularBuffer_Initialize(&vcbSpiTxns[bank], &vcbSpiQueue_data[bank][0],
                                sizeof(spi_xfer_t), SPI_VCB_MAX_ITEMS);
}
 ...
 **
 ******************************************************************************/
hal_status_t HAL_Spi_InitMaster(VOR_SPI_Type* const spi,
  en_spi_clkmode_t mode, uint8_t wordLen, uint16_t clkDiv)
{
  hal_spi_init_t init;
  init.blockmode = true;
  init.bmstall = false;
 ...
 **
 ******************************************************************************/
hal_status_t HAL_Spi_InitSlave(VOR_SPI_Type* const spi,
  en_spi_clkmode_t mode, uint8_t wordLen, uint8_t ss)
{
  hal_spi_init_t init;
 ...
 **
 ******************************************************************************/
hal_status_t HAL_Spi_XferB16(VOR_SPI_Type* const spi,
  uint16_t* txBuf, uint16_t* rxBuf,
 ...
 **
 ******************************************************************************/
hal_status_t HAL_Spi_XferB8(VOR_SPI_Type* const spi,
  ...
  switch(states[bank]){
    ...
    case en_spi_state_finished:
      ...
      if(VOR_CircularBuffer_IsEmpty(&vcbSpiTxns[bank])){
        ...
        if(((VOR_SPI->BANK[bank].CTRL1 & SPI_CTRL1_SS_Msk) >> SPI_CTRL1_SS_Pos) != currentXfer[bank].ss){
...
{
  SPI_StateMachine(SPI0_BANK);
}

void SPI1_RX_IRQHandler (void)
{
  SPI_StateMachine(SPI1_BANK);
}

void SPI2_RX_IRQHandler (void)
{
  SPI_StateMachine(SPI2_BANK);
}

void SPI3_RX_IRQHandler (void)
{
  SPI_StateMachine(SPI3_BANK);
}

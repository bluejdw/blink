...
int calcPLL (uint32_t inFreqHz, uint32_t outFreqHz)
{
  ...
  //jdw dbgprintf ("Begin PLL settings calculation\n");

	if (fout * NO_MIN > VCO_MAX) {
		//jdw dbgprintf ("Error:  Maximum frequency exceeded.\n");
    return -1;
	}
	if (fout * NO_MAX < VCO_MIN) {
		//jdw dbgprintf ("Error:  Minimum frequency exceeded.\n");
    return -1;
	}

	first = firstx = 1;
	if (terr == -1) {
		//jdw dbgprintf ("NR\tNF\tOD\tNB\tFvco\t\terror\n");
		//jdw dbgprintf ("------------------------------------------------------\n");
	} else if (terr != -2) {
		first = 0;
	...
	for (nfi = (int)val; nfi < NF_MAX; ++nfi) {
		...
		if ((first) || (fabs (err) < merr * (1 + 1e-6)) || (fabs (err) < 1e-16)) {
			...
			merr = fabs (err);
			if (terr != -1) continue;
			//jdw dbgprintf ("%d\t%lld\t%d\t%d\t%e\t%#+g\n", nrx, nfx, no, nb, fvco, err);
		}
	}
	if (! found) {
		//jdw dbgprintf ("Error:  No workable settings found.\n");
    return -1;
	}
	if (terr != -1) {
		nrx = x_nrx;
		...
		if ((terr != -2) && (fabs (err) >= terr * (1 - 1e-6))) {
		       //jdw	dbgprintf ("Error:  No appropriate ratio found.\n");
      return -1;
		}

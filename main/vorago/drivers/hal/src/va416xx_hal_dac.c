...
{
  if((dac != VOR_DAC0) && (dac != VOR_DAC1)){ return hal_status_badParam; }
  if(value > 0xfff){ return hal_status_badParam; }
  ...
  __NOP();
  __NOP();
  while (dac->STATUS & DAC_STATUS_DAC_BUSY_Msk);  // wait until not busy
 ...
 **
 ******************************************************************************/
uint32_t HAL_DAC_ReadPerid(VOR_DAC_Type * const dac)
{     
  return dac->PERID;
}

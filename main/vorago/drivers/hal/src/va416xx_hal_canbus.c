...
//The CIEN register enables the transmit and receive interrupts of message buffers 0 through 14 and the CAN error interrupt.		
//    VOR_CAN->BANK[CANPortNum].CIEN = (0x0000ffff);
    VOR_CAN->BANK[CANPortNum].CIEN =
		    0x7FFF<<CAN_CIEN_IEN_Pos 
		    |CAN_CIEN_EIEN_Msk;
				
 //The CICEN register determines whether the interrupt pending flag in IPND should be translated into the Interrupt Code field of the STPND register.      
...
// now the data will change on the next read
    tmpregdata= VOR_CAN->BANK[CANPortNum].CNSTAT_CMB0;
		...
		//this does not appear in Prog Guide, is this a bitband addr for something in the NVIC?
    VOR_CAN->BANK[CANPortNum].CICLR = (0x00000001);
    VOR_CAN->BANK[CANPortNum].DATA3_CMB0 = (0x00003d7f);
    VOR_CAN->BANK[CANPortNum].DATA2_CMB0 = (0x00000f98);
